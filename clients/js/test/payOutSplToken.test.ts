...
import { BGL_GAME_POT_PROGRAM_ID, GamePot, Key, createPotV1, fetchGamePot, findGamePotPda, payInSplTokenV1, payOutSplTokenV1 } from '../src';
import { createUmi } from './_setup';
import { createAndMint, fetchDigitalAssetWithAssociatedToken, TokenStandard } from '@metaplex-foundation/mpl-token-metadata';

test('it can pay out SPL Token from a pot', async (t) => {
  ...
  // And the participant has the correct SPL Token balance.
  t.is((await fetchDigitalAssetWithAssociatedToken(umi, tokenMint.publicKey, participant.publicKey)).token.amount, 100n);

  // And the pot has the correct SPL Token balance.
  t.is((await fetchDigitalAssetWithAssociatedToken(umi, tokenMint.publicKey, publicKey(gamePotPda))).token.amount, 0n);
});
